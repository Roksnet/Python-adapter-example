...
class XroadClient:
    ...
    def _get_client_data(self, db):
        ...
        try:
            ...
            log.error(f'Config error: there must be xroad.client.{db} or xroad.client with format xRoadInstance/memberClass/memberCode/subsystemCode')
            raise
        else:
            return dict(
    ...
    def _get_server_data(self, db):
        ...
        try:
            ...
            log.error(f'Config error: there must be xroad.server.{db} with format xRoadInstance/memberClass/memberCode/subsystemCode')
            raise
        else:
            return dict(
                xRoadInstance = xRoadInstance,
    ...
    def allowedMethods(self):
        "Ask list of permitted services (only for testing without security server)"
        list_path = ['/service']
        res = self.call('allowedMethods.v1', E.allowedMethods(), list_path)
        items = res and res.service or []
        return items
    
    def call(self, service_name, params, list_path=[], service_version=None, attachments=[]):
        """
        Call X-road service
    ...
    def on_fault(self, e, service_name):
        ...
        log.error(buf)
        err = 'X-road service failed (%s: %s)' % (self.producer, e.faultstring)
    ...
    def send_xml(self, service_name, xml, attachments=[], namespace=None):
        ...
        self._log_msg(service_name, 'in', xml)
        ...
        self._log_msg(service_name, 'out', self.xml_response)
